...
names(correct.answers) <- c("Experiment", "Item", "Condition", "CorrectAnswer1", "CorrectAnswer2")
...
mydata$Answer.YNQ1_[mydata$Answer.YNQ1_==""] <- NA
mydata$Answer.YNQ2_[mydata$Answer.YNQ2_==""] <- NA

#################################################################
## Now add correct answers and condition information

# count NA as incorrect
mydata$Correct <- (as.character(mydata$CorrectAnswer1) == as.character(mydata$Answer.YNQ1_)) &
				  (as.character(mydata$CorrectAnswer2) == as.character(mydata$Answer.YNQ2_))
...
participant.nas <- aggregate(is.na(mydata$Answer.YNQ1) | is.na(mydata$Answer.YNQ2), by=list(mydata$Participant), mean)
...
participant.responsecount <- aggregate(mydata$Answer.YNQ1, by=list(mydata$Participant), length)
